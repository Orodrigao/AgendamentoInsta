...
async function loadBufferChannels() {
  ...
  try {
    const res = await fetch('https://api.bufferapp.com/1/profiles.json?access_token=' + key);
...
async function schedulePost() {
  ...
  try {
    ...
    params.append('scheduled_at', new Date(`${date}T${time}:00`).toISOString());
    params.append('access_token', bufferKey);
